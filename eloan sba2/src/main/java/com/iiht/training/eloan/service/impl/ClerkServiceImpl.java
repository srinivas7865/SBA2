...
public class ClerkServiceImpl implements ClerkService {
	...
	public List<LoanOutputDto> allAppliedLoans() {
		// TODO Auto-generated method stub
		 List<LoanOutputDto> lt = null;
		 lt = loanRepository.findAll().stream().map(e -> EMParser.parse1(e)).collect(Collectors.toList());
		 for(int i =0;i<=lt.size();i++) {			 
				if(!((lt.get(i)).getRemark().equalsIgnoreCase("Applied"))) {
					lt.remove(i);
				}
				}
		 return lt;
